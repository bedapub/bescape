...
#fadista.healthy.ens <- SCDC_ENSEMBLE(bulk.eset = bulk %>% filter(params$conditionvar == params$condition_level1), sc.eset.list = scdata, ct.varname = params$celltypevar, sample = params$samplevar, truep = NULL, ct.sub =  celltypesel, search.length = 0.01, grid.search = T)  
message(Initialting deconvolution. This process may take a while for larger datasets)
fadista.healthy.ens <- SCDC_ENSEMBLE(bulk.eset = bulk, sc.eset.list = scdata, ct.varname = params$celltypevar, sample = params$samplevar, truep = NULL, ct.sub =  celltypesel, search.length = 0.01, grid.search = T)  
...
getProp <- function(ens, metric = NULL){
    out = as.data.frame(wt_prop(ens$w_table[metric, 1:2], ens$prop.only))
    return out
}

#prop <- fadista.healthy.ens$prop.only
prop <- getProp(fadista.healthy.ens, metric=1)
