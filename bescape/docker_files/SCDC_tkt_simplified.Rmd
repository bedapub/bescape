...
library(SCDC)
scdata<-NULL
nscdata<-length(params$scfilelist)
for(index in 1:nscdata){
scdata[[index]]<-readRDS(params$scfilelist[index])
}
...
fadista.healthy.ens <- SCDC_ENSEMBLE(bulk.eset = bulk %>% filter(params$conditionvar == params$condition_level1), sc.eset.list = scdata, ct.varname = params$celltypevar,
sample = params$samplevar, truep = NULL, ct.sub =  params$celltypesel, search.length = 0.01, grid.search = T)  
